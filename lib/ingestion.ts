...
export async function storeOriginal(config:Config,file:File,fileId:string){
  const key=`${new Date().toISOString().slice(0,10)}/${fileId}/${sanitize(file.name)}`;
...
function normalizeHeader(v:string){return v.trim().toLowerCase().replace(/\s+/g,'').replace(/[()]/g,'');}function toText(v:unknown){return v instanceof Date?v.toISOString():v==null?'':String(v).trim();}function sanitize(v:string){return v.normalize('NFKC').replace(/[^\p{L}\p{N}._-]+/gu,'_');}function mimeFromName(n:string){return n.toLowerCase().endsWith('.pdf')?'application/pdf':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';}async function sha256(file:File){const digest=await crypto.subtle.digest('SHA-256',await file.arrayBuffer());return Array.from(new Uint8Array(digest),b=>b.toString(16).padStart(2,'0')).join('');}
