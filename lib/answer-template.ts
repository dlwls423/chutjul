...
export function formatComplaintAnswer(input: {
  ...
  const sectionText = input.sections
    .slice(0, Math.min(input.maxSections || 4, KOREAN_SECTION_LABELS.length))
    .map(
      (section, index) =>
  ...
  return [
    `1. 안녕하십니까. ${input.organizationName || "개인정보보호위원회"}입니다. 귀하께서 국민신문고를 통해 신청하신 민원(신청번호: ${applicationNumber})에 대한 검토 결과를 다음과 같이 알려드립니다.`,
    `2. 귀하께서는 ${inquirySummary}에 대해 문의하신 것으로 이해됩니다.`,
    sectionText,
    `3. 답변 내용에 대한 추가 설명이 필요한 경우 ${input.organizationName || "개인정보보호위원회"} ${input.department} ${input.userName} ${input.position} (☏${phone})으로 ${input.closingMessage || "연락주시면 친절히 안내해 드리도록 하겠습니다. 감사합니다. 끝."}`,
  ]
    .filter(Boolean)
    .join("\n\n");
}
