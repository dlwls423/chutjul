...
export async function searchCurrentLaws(rawQuery:string,department:string){
  ...
  return provisions.map(p=>{const s=sourceMap.get(p.source_id);if(!s)return null;const lawName=norm(s.canonical_name);const heading=norm(p.heading||"");const body=norm(p.body);const hay=`${lawName} ${heading} ${body}`;const matched=terms.filter(t=>hay.includes(t));if(!matched.length)return null;const dept=Array.isArray(s.related_departments)?s.related_departments.map(String):[];const coverage=matched.length/terms.length;const nameHits=matched.filter(t=>lawName.includes(t)).length;const headingHits=matched.filter(t=>heading.includes(t)).length;const articleHit=/제\s*\d+\s*조/.test(rawQuery)&&norm(`${p.article_number||""} ${p.body}`).includes(norm(rawQuery).match(/제\s*\d+\s*조/)?.[0]||"");const score=Math.round(coverage*120)+matched.length*18+nameHits*80+headingHits*35+(articleHit?120:0)+(s.priority||0)+(dept.includes("전체")||dept.includes(department)?25:0);const v=versionMap.get(p.version_id);return{id:`law:${p.id}`,title:`${s.canonical_name}${p.article_number?` ${p.article_number}`:""}`,documentType:"law",department:"공통 법령",category:"현행 법령",createdAt:v?.fetched_at||"",snippet:clip(`${p.heading||""} ${p.body}`,matched),question:"",answer:"",content:p.body,score,matchedTerms:matched,legalReferences:[`${s.canonical_name} ${p.article_number||""}`.trim()],complaintMetadata:{effective_from:v?.effective_from,proclamation_number:v?.proclamation_number,source_url:s.source_url},guideMatches:[]};}).filter((v):v is NonNullable<typeof v>=>Boolean(v)).sort((a,b)=>b.score-a.score).slice(0,40);
}
